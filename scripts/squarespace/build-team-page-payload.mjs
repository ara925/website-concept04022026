...
import { teamBannerImage, teamHeroImage, teamMembers } from "./team-data.mjs";
...
const cardsMarkup = teamMembers
  .map(
    (member) => `
        <a class="team-card" href="/${member.slug}">
          <img id="${member.key}" src="${member.cardImage}" alt="${member.title}">
          <div class="team-card-tag">View Profile</div>
          <div class="team-card-gradient"></div>
          <div class="team-card-info">
            <h3 style="color:#ffffff">${member.title}</h3>
            <p>${member.role}</p>
          </div>
        </a>`,
  )
...
.sq-wrap{--bg:#0a0f1c;--bg-card:#0f1525;--fg:#e2e8f0;--fg-muted:#8a9bb5;--primary:#3b82f6;--border:#1e2a3f;font-family:'Inter',system-ui,sans-serif;background:var(--bg);color:var(--fg);line-height:1.5;-webkit-font-smoothing:antialiased}
.sq-wrap *,.sq-wrap *::before,.sq-wrap *::after{margin:0;padding:0;box-sizing:border-box}
.sq-wrap a{color:inherit;text-decoration:none}.sq-wrap img{max-width:100%;display:block}.sq-wrap ul{list-style:none}
...
.sq-wrap .section{padding:96px 48px}.sq-wrap .section-inner{max-width:1400px;margin:0 auto}
.sq-wrap .photo-banner{position:relative;height:40vh;overflow:hidden}.sq-wrap .photo-banner img{width:100%;height:100%;object-fit:cover}
...
@media (max-width:768px){.sq-wrap .team-grid{grid-template-columns:1fr!important}.sq-wrap .section{padding:48px 20px}.sq-wrap .hero-content{padding:0 20px 40px}}
@media (min-width:769px) and (max-width:1024px){.sq-wrap .team-grid{grid-template-columns:repeat(2,1fr)!important}}
#jeff{object-position:0 -48px}#cullen{object-position:0 -10px}#justin{object-position:0 -10px}#bill{object-position:0 0}
</style>

<div class="sq-wrap" data-decker-team-page="true">
  <section class="hero">
    <div class="hero-bg">
      <img src="${teamHeroImage}" alt="Team photo of Decker Healthcare Group">
    </div>
...
${cardsMarkup}
  ...
  <section class="photo-banner">
    <img src="${teamBannerImage}" alt="Senior living facility interior">
