export type MarketTrendReport = {
  slug: string;
  title: string;
  issueLabel: string;
  category: "Quarterly Report" | "Market Update";
  isoDate: string;
  pageCount: number;
  summary: string;
  highlights: string[];
...
export const marketTrendReports: MarketTrendReport[] = [
  {
...
export const featuredMarketTrendReport = marketTrendReports[0];

export const marketTrendArchiveByYear = marketTrendReports.reduce<Record<string, MarketTrendReport[]>>((acc, report) => {
  const year = report.isoDate.slice(0, 4);
  if (!acc[year]) {
    acc[year] = [];
  }
